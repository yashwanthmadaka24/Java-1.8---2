...
public class Ch2App6Exercises {
	...
	public static void main(String[] args) {
		...
		List<Integer> values = Arrays.asList(1, 3, 5, 9, -500);
		int finalValue = values.get(0);
		for (int i = 1; i < values.size(); i++) {
			finalValue = finalValue - values.get(i);
		}
		System.out.println(finalValue);
		// @formatter:off
		System.out.println(Stream.of(1, 3, 5, 9, -500)
		    .reduce((x, y) -> x - y));
		...
		/*
		 * Write a function that takes in HealthPlans and returns the list of Strings
		 * with their names & applicable state
		 */

		System.out.println(getHealthPlanStates(HealthData.getThreeHealthPlans()));

		/*
		 * Write a function that takes Employees and return the list of Employees with
		...
		long count = HealthData.threeHealthPlans().flatMap(x -> x.getBenefits()).count();
		System.out.println("Total Health Benefits offered are : " + count);

		long distinctCount = HealthData.threeHealthPlans().flatMap(x -> x.getBenefits()).distinct().count();
...
//
		...
		System.out.println("The string with max lowercase " + "characters is : " + maxCountString2.get() + " with : "
				+ maxCount2 + " lower case chars");

	}

	static List<Employee> getEmployeesWith2Dependents(Stream<Employee> allEmployees) {

		List<Employee> empsWithAtLeast2Dependents = allEmployees.filter(x -> x.getDependentList().size() >= 2)
	...
	static List<String> getHealthPlanStates(List<HealthPlanGenericImpl> healthPlans) {
		...
		List<String> names = healthPlans.stream().map(x -> x.getName() + " - " + x.getState())
				.collect(Collectors.toList());
	...
	static int addUp(Stream<Integer> numbers) {
		...
		int sum = numbers.reduce(0, (accumulator, element) -> accumulator + element);
